...
const Progress = ({ value = 0 }: { value?: number }) => (
  <ProgressBar value={value} maxValue={100} aria-label="progress">
...
export function AnalyticsScreen({ onNavigate }: AnalyticsScreenProps) {
  ...
  const avgProcessingTime = (() => {
    const done = myApps.filter(a => a.currentStatus === 'approved' && a.createdAt && a.updatedAt);
  ...
  const processWarnings = myApps
    .filter(a => a.currentStatus === 'more_info')
    .map(a => ({
      id:         a.id,
