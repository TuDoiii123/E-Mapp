...
const Badge = ({ variant: _v, className = '', children }: any) => (
  <Chip variant="flat" size="sm" className={className}>{children}</Chip>
);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="px-4 pb-6">
        <Tabs defaultSelectedKey="notifications" className="w-full">
          {/* Tab List */}
          <Tabs.List className="grid w-full grid-cols-2 h-12 bg-gray-100 rounded-xl p-1 mb-4">
            <Tabs.Tab id="notifications" className="rounded-lg font-medium">
              Thông báo ({unreadCount})
            </Tabs.Tab>
            <Tabs.Tab id="history" className="rounded-lg font-medium">
              Lịch sử ({history.length})
            </Tabs.Tab>
          </Tabs.List>

          {/* Notifications panel */}
          <Tabs.Panel id="notifications" className="space-y-4">
            {showFilter && (
              <select
                value={filter}
                onChange={e => { setFilter(e.target.value); setShowFilter(false); }}
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        <div className="flex items-start justify-between mb-1">
                          <h3 className={!notification.read ? '' : 'text-gray-700'}>{notification.title}</h3>
                          <div className="flex items-center gap-2">
          ...
          <Tabs.Panel id="history" className="space-y-4">
                ...
                >
                  <CardContent className="p-4">
                    <div className="flex items-center justify-between mb-2">
                      <h3>{item.title}</h3>
                      <Badge className={getStatusColor(item.statusKey)}>{item.status}</Badge>
                    </div>
            ...
            <Button variant="outline" className="w-full" onClick={() => onNavigate('submit')}>
              Nộp hồ sơ mới
            </Button>
          </Tabs.Panel>
        </Tabs>
